...
namespace ICSharpCode.NRefactory.PrettyPrinter
{
	public sealed class CSharpOutputVisitor : NodeTrackingAstVisitor, IOutputAstVisitor
	{
		...
		public override object TrackedVisitConstructorDeclaration(ConstructorDeclaration constructorDeclaration, object data)
		{
            ...
            foreach (INode child in constructorDeclaration.Body.Children)
            {
                if (child is ExpressionStatement &&
                    ((ExpressionStatement)child).Expression is InvocationExpression)
                {
                    Expression targetEx = ((InvocationExpression)((ExpressionStatement)child).Expression).TargetObject;
                    if (targetEx is MemberReferenceExpression)
                    {
                        MemberReferenceExpression newStatement = (MemberReferenceExpression)targetEx;
                        if (newStatement.MemberName == "New")
                        {
                            PatchedVisitConstructorInitializer(newStatement, data);
                            constructorDeclaration.Body.Children.Remove(child);
                            break;
                        }
                    }
                }
        ...
        //BEGIN MODIFIED DENIZ ESEN
        public void PatchedVisitConstructorInitializer(MemberReferenceExpression newStatement, object data)
        {
            outputFormatter.Space();
            outputFormatter.PrintToken(Tokens.Colon);
            outputFormatter.Space();
            if (newStatement.TargetObject is BaseReferenceExpression)
            {
            ...
            AppendCommaSeparatedList( ((InvocationExpression)newStatement.Parent).Arguments );
            if (this.prettyPrintOptions.WithinMethodCallParentheses)
            {
                outputFormatter.Space();
            }
